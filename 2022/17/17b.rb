...
NUM_ROCKS.times do |i|
	...
	if i % LIMIT == 0
		# compat
		CHAMBER.rotate(SIZE/3)
		0.upto(SIZE/3-1) { |j| CHAMBER[j] = "." }
	end
	...
	loop do
		...
		collision = proyection.each.any? { |p| 
			p0 = (p[1] + 1) * WIDTH + p[0]
			p[1] == HEIGHT - 1 or CHAMBER[p0] == "#" # TODO
		}
		if collision
			proyection.each do |p|
				CHAMBER[p[1] * WIDTH + p[0]] = "#"
				initial_y = p[1] if p[1] < initial_y # TODO
			end
			break
		end
